<x-app-layout>
    ...
    <div class="py-6 sm:py-12" x-data="{ 
        ...
        updateTotal: function() {
            // Pastikan input kuantitas valid sebelum perhitungan
        ...
        openOrderModal: function(post) {
            ...
            // Set kuantitas awal: gunakan minOrder jika minOrder > 0, atau default ke 1
            this.tempKuantitas = post.minOrder > 0 ? post.minOrder : 1; 
            
            // Jika stok lebih kecil dari minOrder, set kuantitas ke stok
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded relative mb-4 sm:mb-6 shadow-md" role="alert">
                ...
                <span class="block sm:inline text-sm">Anda belum melengkapi data profil. Lengkapi data **Profil & Alamat** untuk bisa memulai transaksi.</span>
                <div class="mt-2">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-4 sm:p-6">
                ...
                <form method="GET" action="{{ route('dashboard') }}" class="mb-4 sm:mb-6">
                    ...
                    <div class="relative">
                        ...
                        <button type="submit" class="absolute inset-y-0 right-0 flex items-center pr-3">
                            <svg class="w-4 h-4 sm:w-5 sm:h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                        </button>
                    </div>
                ...
                @if ($postingan_dagangans->isEmpty() && $q)
                    <p class="text-center text-gray-500 mt-6 sm:mt-8">Tidak ditemukan penawaran untuk kata kunci **"{{ $q }}"**.</p>
                ...
                @else
                    <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6"> 
                        ...
                        @foreach ($postingan_dagangans as $postingan)
                            <div class="bg-white border border-gray-200 rounded-lg shadow-md hover:shadow-xl transition duration-300">
                                {{-- Gambar Produk --}}
                                @php
                                    $fotoUrl = $postingan->foto_postingan ? asset('storage/' . $postingan->foto_postingan) : 'https://placehold.co/600x400/CCCCCC/white/PRODUK';
                                @endphp
                                ...
                                <div class="p-3 sm:p-4">
                                    ...
                                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center text-xs text-gray-600 mb-3 sm:mb-4">
                                        <div class="flex items-center mb-1 sm:mb-0">
                                            <svg class="w-3 h-3 mr-1 text-red-500 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd"></path></svg>
                                    ...
                                    {{-- Tombol Aksi (Lihat Detail & Beli - Memanggil Modal) --}}
                                    ...
                                    href="#"
                                    class="inline-flex items-center px-2 py-1 sm:px-4 sm:py-2 text-xs sm:text-sm font-medium text-center text-white bg-green-600 rounded-lg focus:ring-4 focus:outline-none focus:ring-green-300 transition duration-150 ease-in-out w-full justify-center 
                                    @if (!$detailPedagang) 
                                        opacity-50 cursor-not-allowed hover:bg-green-600" onclick="alert('Lengkapi profil Anda terlebih dahulu!')"
                                    @else
                                        hover:bg-green-700"
                                    @endif
                                    >
                                ...
                                <div class="mt-2">
                                    ...
                                    <form :action="'{{ route('dashboard.pedagang.pesanan.store', ['postingan' => 'TEMP_ID']) }}'.replace('TEMP_ID', postId)" method="POST">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            {{-- Tampilkan error validasi Laravel setelah submit --}}
